...
import hu.bme.aut.mobillab_android.R
import hu.bme.aut.mobillab_android.databinding.DialogDetailBinding
...
class DetailDialogFragment : DialogFragment() {
    ...
    interface DetailDialogListener{
        fun onDismissCalled(favStateOnDismiss: Boolean, pokemonId: Int)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.cbIsFav.setOnCheckedChangeListener { _, isChecked ->
            if(isChecked){
                viewModel.addPokemonToFavourites(poke!!.id)
            }
            else{
                viewModel.removePokemonFromFavourites(poke!!.id)
            }
        }
    }
    ...
    override fun onDismiss(dialog: DialogInterface) {
        ...
        listener.onDismissCalled(binding.cbIsFav.isChecked, arguments?.getParcelable<Poke>(KEY_POKE)!!.id)
    }
